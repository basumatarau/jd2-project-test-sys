			...
			<tbody>
				<c:forEach items="${assignmentList}" var="assignment">
					<tr>
						...
						<td>${assignment.assignee.firstName}
							${assignment.assignee.lastName}</td>
						...
						<td><c:choose>
								<c:when test="${assignment.isSubmitted()}">
									<form class="form-horizontal"
										action="${pageContext.request.contextPath}/assignment-manager/showResult"
										method="get">
										<button id="show-result" value="${assignment.id}" name="id"
											class="btn btn-primary">Show results</button>
									</form>
								</c:when>
								<c:when test="${!assignment.isSubmitted()}">
									<form class="form-horizontal"
										action="${pageContext.request.contextPath}/assignment-manager/delete?id=${assignment.id}"
										method="post">
			...
			<ul>
				<c:forEach begin="${startpage}" end="${endpage}" var="p">
					<li class="page-item"><a
						href="${pageContext.request.contextPath}/assignment-manager?page=${p}">${p}</a>
					</li>
				</c:forEach>
			</ul>
		</div>
